...
def main():
    # Generate BGKxxx scripts in deterministic order
    os.makedirs(THIS_DIR, exist_ok=True)
    submit_lines = ["#!/bin/bash", "set -e", "# Submit all BGK jobs in order"]
    idx = 1
    ...
    table_path = os.path.join(THIS_DIR, "BGK_table.md")
    ...
    submit_path = os.path.join(THIS_DIR, "BGK_submit_all.sh")
