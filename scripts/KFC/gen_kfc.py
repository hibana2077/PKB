...
def main():
    # Generate KFCxxx scripts in deterministic order
    os.makedirs(THIS_DIR, exist_ok=True)
    submit_lines = ["#!/bin/bash", "set -e", "# Submit all KFC jobs in order"]
    idx = 1
    ...
    submit_path = os.path.join(THIS_DIR, "KFC_submit_all.sh")
