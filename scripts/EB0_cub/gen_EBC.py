...
def main():
    # Generate EBCxxx scripts in deterministic order
    os.makedirs(THIS_DIR, exist_ok=True)
    submit_lines = ["#!/bin/bash", "set -e", "# Submit all EBC jobs in order"]
    idx = 1
    ...
    table_path = os.path.join(THIS_DIR, "EBC_table.md")
    ...
    submit_path = os.path.join(THIS_DIR, "EBC_submit_all.sh")
